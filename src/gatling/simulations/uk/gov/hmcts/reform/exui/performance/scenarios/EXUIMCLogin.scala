...
import uk.gov.hmcts.reform.exui.performance.scenarios.utils.LoginHeader.headers_health_check
...
object EXUIMCLogin {
  ...
  val caseworkerLogin =
    group("XUI${service}_020_SignIn") {
      exec(http("XUI${service}_020_005_SignIn")
        ...
        .exec(http("XUI${service}_020_020_HealthCheck")
          .get("/api/healthCheck?path=%2Fcases")
          .headers(headers_health_check)
          .check(headerRegex("set-cookie", "XSRF-TOKEN=(.*);").saveAs("xsrfToken1"))
          .check(status.in(200, 304)))

        .repeat(1, "count") {
          exec(http("XUI${service}_020_025_AcceptT&CAccessJurisdictions${count}")
            .get("/aggregated/caseworkers/:uid/jurisdictions?access=read")
            .headers(LoginHeader.headers_access_read_caseworker)
            .check(status.in(200, 304, 302)))
        }

        .exec(getCookieValue(CookieKey("XSRF-TOKEN").withDomain(baseDomain).saveAs("xsrfToken2")))

        .exec(http("XUI${service}_020_030_GetWorkBasketInputs")
          .get("/data/internal/case-types/FinancialRemedyMVP2/work-basket-inputs")
          .headers(LoginHeader.headers_17_caseworker))


        /* .exec(http("XUI${service}_020_030_GetDefaultWorkBasketView")
               .get("/data/internal/searchCases?ctid=FinancialRemedyConsentedRespondent&use_case=WORKBASKET&view=WORKBASKET&state=caseAdded&page=1")
               .headers(LoginHeader.headers_0))*/
  ...
  * currently XUI disable the toggle for below requests
  ...
  exec(http("XUI${service}_030_005_ConfirmT&C")
    .get(manageOrgURL + "/api/user/details")
    .check(status.in(200, 302,304))
    .check(jsonPath("$..userId").optional.saveAs("myUserId")))

    .exec(http("XUI${service}_030_010_ConfirmT&C")
      .post(manageOrgURL + "/api/userTermsAndConditions")
      .headers(LoginHeader.headers_tc)
      .body(StringBody("{\"userId\":\"${myUserId}\"}"))
      .check(status.in(200, 304, 302))).exitHereIfFailed

      .exec(http("XUI${service}_030_015_AcceptT&CEnabled")
        .get(manageOrgURL + "/api/configuration?configurationKey=termsAndConditionsEnabled")
        .headers(LoginHeader.headers_hometc)
        .check(status.in(200, 304, 302)))

      .repeat(1, "count") {
        exec(http("XUI${service}_030_020_AcceptT&CAccessJurisdictions${count}")
          .get(manageOrgURL + "/aggregated/caseworkers/:uid/jurisdictions?access=read")
          .headers(LoginHeader.headers_access_read)
          .check(status.in(200, 304, 302)))
      }

      .exec(http("XUI${service}_030_025_GetWorkBasketInputs")
        .get(manageOrgURL + "/data/internal/case-types/CARE_SUPERVISION_EPO/work-basket-inputs")
        .headers(LoginHeader.headers_17))

      .exec(http("XUI${service}_030_030_GetPaginationMetaData")
        .get(manageOrgURL + "/data/caseworkers/:uid/jurisdictions/PUBLICLAW/case-types/CARE_SUPERVISION_EPO/cases/pagination_metadata?state=Open")
        .headers(LoginHeader.headers_0))

      .exec(http("XUI${service}_030_035_GetDefaultWorkBasketView")
        .get(manageOrgURL + "/aggregated/caseworkers/:uid/jurisdictions/PUBLICLAW/case-types/CARE_SUPERVISION_EPO/cases?view=WORKBASKET&state=Open&page=1")
        .headers(LoginHeader.headers_0))
