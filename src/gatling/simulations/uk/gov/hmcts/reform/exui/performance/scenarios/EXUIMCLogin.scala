...
object EXUIMCLogin {
  ...
  *=====================================================================================*/
  ...
  val manageCasesHomePage =
    group("XUI_FR_010_Homepage") {
      exec(http("XUI_FR_010_005_Homepage")
        .get("/")
        .headers(LoginHeader.headers_0)
        .check(status.in(200, 304))).exitHereIfFailed

        .exec(http("XUI_FR_010_010_HomepageConfigUI")
          .get("/external/configuration-ui")
          .headers(LoginHeader.headers_1))

        .exec(http("XUI_FR_010_015_HomepageConfigJson")
          .get("/assets/config/config.json")
          .headers(LoginHeader.headers_1))

        .exec(http("XUI_FR_010_020_HomepageTCEnabled")
          .get("/api/configuration?configurationKey=termsAndConditionsEnabled")
          .headers(LoginHeader.headers_1))

        .exec(http("XUI_FR_010_025_HomepageIsAuthenticated")
          .get("/auth/isAuthenticated")
          .headers(LoginHeader.headers_1))

        .exec(http("XUI_FR_010_030_AuthLogin")
          .get("/auth/login")
  ...
  * service related journeys like divorce,fpla,iac,FR etc...
  =====================================================================================*/


  val manageCaseslogin =
    group("XUI_FR_020_005_SignIn") {
      exec(flushHttpCache).exec(http("XUI_FR_020_005_SignIn")
        ...
        .exec(http("XUI_FR_020_010_configUI")
          .get("/external/config/ui")
          .headers(LoginHeader.headers_0)
          .check(status.in(200, 304)))

        .exec(http("XUI_FR_020_015_Config")
          .get("/assets/config/config.json")
          .headers(LoginHeader.headers_0)
          .check(status.in(200, 304)))

        .exec(http("XUI_FR_020_020_SignInTCEnabled")
          .get("/api/configuration?configurationKey=termsAndConditionsEnabled")
          .headers(LoginHeader.headers_38)
          .check(status.in(200, 304)))

        .exec(http("XUI_FR_020_025_SignInGetUserId")
          .get("/api/user/details")
          .headers(LoginHeader.headers_0)
          .check(status.in(200, 304)))

        .repeat(1, "count") {
          exec(http("XUI_FR_020_030_AcceptT&CAccessJurisdictions${count}")
        ...
        .exec(http("XUI_FR_020_045_CaseActivity")
          .options("/activity/cases/0/activity")
          .headers(Headers.commonHeader)
          .check(status.in(200, 304, 403)))

        .exec(http("XUI_FR_020_050_CaseActivity")
          .get("/activity/cases/0/activity")
          .headers(Headers.commonHeader)
          .check(status.in(200, 304, 403)))

        .exec(http("XUI_FR_020_055_GetDefaultWorkBasketView")
        ...
        .doIf("${caseList.exists()}") {
          exec(http("XUI_FR_020_060_CaseActivity")
  ...
  val manageCase_Logout =
    ...
    group("XUI_FR_999_SignOut") {
      exec(http("XUI_FR_999_SignOut")
        .get("/api/logout")
